...
public class VCRTV : MonoBehaviour
{
    ...
    public GameObject TriggerAristóteles;
    public Electricity ele;
    Electricity QuadroEletrico;
    [HideInInspector]
    public VCRPlayer VCR;

    [Header("Main")]
    public VideoPlayer videoPlayer;
    public VideoPlayer videoPlayerVisor;
    ...
    void Awake()
    {
        ...
        videoPlayerVisor.enabled = false;
        TriggerAristóteles.SetActive(false);
    ...
    void Start()
    {
        ...
        QuadroEletrico = GetComponent<Electricity>();
    ...
    void Update()
    {
        ...
        if (ele.isPoweredOn==true)
        {
            PowerButton.gameObject.GetComponent<Collider>().enabled = true;
            
        }
        else
        {
            PowerButton.gameObject.GetComponent<Collider>().enabled = false;
    ...
    public void PowerOnOff(bool clickSound = true)
    {
        if (!isOn)
        {
            ...
            EnableScreen(true);
            isOn = true;
            videoPlayerVisor.enabled = true;
            TriggerAristóteles.SetActive(true);
            float randomNumber = Random.Range(0, 20);
            if (randomNumber == 2)
            {
                videoPlayerVisor.clip = canais[1];
            }
            else
            {
                videoPlayerVisor.clip = canais[0];
            }
