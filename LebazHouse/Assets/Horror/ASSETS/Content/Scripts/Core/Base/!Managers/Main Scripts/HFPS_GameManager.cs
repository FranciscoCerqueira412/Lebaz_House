...
public class HFPS_GameManager : Singleton<HFPS_GameManager> {
    ...
    /// <param name="Interagir">Interact Enabled State</param>
    ...
    public void AddPickupMessage(string itemName)
    {
        ...
        Message.GetComponent<Notification>().SetMessage(string.Format("Apanhaste {0}", itemName));
    }
    ...
    public void ShowExamineSprites(bool btn1 = true, bool btn2 = true, bool btn3 = true, bool btn4 = true, string PutAwayText = "Pousar", string UseText = "Levar")
    {
        ...
        if (btn3) { SetKey(HelpButton3.transform, RotateKey, "Girar"); } else { HelpButton3.SetActive(false); }
        if (btn4) { SetKey(HelpButton4.transform, CursorKey, "Mostrar cursor"); } else { HelpButton4.SetActive(false); }
        DownHelpUI.SetActive(true);
    }

    public void ShowPaperExamineSprites(CrossPlatformControl ExamineKey, bool rotate, string ExamineText = "Examine")
    {
        SetKey(HelpButton1.transform, GrabKey, "Pousar");
        SetKey(HelpButton2.transform, ExamineKey, ExamineText);

        if (rotate)
        {
            SetKey(HelpButton3.transform, RotateKey, "Girar");
        }
        else
        {
    ...
    {
        SetKey(HelpButton1.transform, GrabKey, "Pousar");
        SetKey(HelpButton2.transform, RotateKey, "Girar");
        SetKey(HelpButton3.transform, ThrowKey, "Atirar");
        HelpButton4.SetActive(false);
        DownHelpUI.SetActive(true);
    }
